...
class BottomNavbarActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bottomNavigation.add(CurvedBottomNavigation.Model(1, "", R.drawable.ic_dashboard1))
        bottomNavigation.add(CurvedBottomNavigation.Model(2, "", R.drawable.ic_history1))
        bottomNavigation.add(CurvedBottomNavigation.Model(3, "", R.drawable.ic_add1))
        bottomNavigation.add(CurvedBottomNavigation.Model(4, "", R.drawable.ic_foryou2))
        bottomNavigation.add(CurvedBottomNavigation.Model(5, "", R.drawable.ic_profile1))

        bottomNavigation.setOnClickMenuListener {
            when (it.id) {
                1 -> replaceFragment(HomeFragment())
                2 -> replaceFragment(HistoryFragment())
                3 -> showAddTransactionOptions()
                4 -> replaceFragment(ForYouDummyFragment())
                5 -> replaceFragment(ProfileDummyFragment())
            }
        }
    ...
    private fun showAddTransactionOptions() {
        val bottomSheet = AddOptionBottomSheet { selectedOption ->
            when (selectedOption) {
                "automatic" -> loadTransactionFragment(AutomaticTransactionFragment())
                "manual" -> loadTransactionFragment(ManualTransactionFragment())
            }
        }
